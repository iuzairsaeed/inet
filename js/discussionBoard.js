...
function threadPostPagination(page = 1) {
    ...
    $.ajax({
        type: "POST",
        url: `${base_url}thread/posts/pagination`,
        dataType: "json",
        contentType: "application/json",
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
                                ...
                                <div class="col-lg-10 col-md-9 mb-3">
                                    <div class="col-md-12 bg-lightWhite100 p-0 font-familyAtlasGroteskWeb-Regular text-colorblue200 font-size13px arrow">
                                        <div class="col-md-12 p-4">
                                            ...
                                            <div class="p-4 post_body"> ${thread_posts[i].body} </div>
                                        </div>
...
$("#post-form").submit(function (e) {
    ...
    var plainHtml = $($("#summernote").summernote("code")).html()

    $("#post-form-err").html("")

    if ((plainText.trim() == null || plainText.trim() == "") && plainHtml.search("<img src") == -1) {
        e.preventDefault();
        $("#summernote").summernote("focus");
